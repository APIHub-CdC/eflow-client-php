...
namespace EFLOW\Client\Interceptor;
...
use \EFLOW\Client\Interceptor\KeyHandler;
use \EFLOW\Client\Interceptor\MyLogger;
use \EFLOW\Client\Model\Errors;
use \EFLOW\Client\Model\Error;

class MiddlewareEvents
{
    
    function __construct(\EFLOW\Client\Interceptor\KeyHandler $signer)
    {
        $this->signer = $signer;
...
function build_error($code, $message){
    $error = new \EFLOW\Client\Model\Error([
        "code" => $code,
        "message" => $message
    ]);
    $errors = new \EFLOW\Client\Model\Errores(["errors" => [$error]]);
